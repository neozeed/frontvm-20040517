...
int asm_pass1 ()
{
	...
	while (fgets (buf, sizeof (buf), fin)) {
			...
			while (isdigit (*pos) || isalpha (*pos) || (*pos == '$')) {
				...
				if (imm.has_label) {
					switch (size) {
						...
						case LONG: default:
							wr_int (0);
							add_fixup (get_bitpos()-4, LONG_FIXUP, imm.label);
							break;
					}
				} else {
					switch (size) {
		...
		/* DIVS/DIVU */
		if (strncmp (pos, "div", 3) == 0) {
			pos += 3;
			if (pos[0] == 's') op.code = 0x81c0;
			else op.code = 0x80c0;
			pos++;
			check_whitespace();
...
blork:
		...
		if (strncmp (pos, "bsr", 3) == 0) {
			...
			wr_short (op.code);
			if (size == 's') {
				add_fixup (get_bitpos()-1, BYTE, lab1);
			} else if (size == 'w') {
				wr_short (0);
				add_fixup (get_bitpos()-2, WORD, lab1);
			} else {
...
int main (int argc, char **argv)
{
	int size, num;
	if (argc == 1) {
		printf ("Usage: ./as68k [--dump-labels] file.s\n");
		exit (0);
	}
	...
	printf ("Done! %d bytes and %d relocations.\n", size, num);
	return 0;
}
