...
#define F_LABEL_TABLE	(1<<7)
#define F_LABEL_MAJOR	(1<<8)
...
static void dump_code ()
{
	...
	while (buf_pos < end) {
		...
		if (labels[buf_pos] & F_LABEL_TABLE) {
...
skip_simple:
	...
	for (pos=base, dist=0; pos<end; pos++) {
		...
		if (dist == 5) {
			labels [pos-4] |= F_GUESS_STRING;
			labels [pos-3] |= F_GUESS_STRING;
...
int main (int argc, char **argv)
{
	...
	if (argc < 2) {
		printf ("Usage: ./thing {-gs|-gw {-codestart addr, addr, ... } {-codestop addr, addr, ... } { -labeltab from,to from2,to2 ... } some.prg\n");
		...
		printf ("    data.\n");
		printf (" -labeltab is for raw reloc thingies. ie dc.l label, which\n");
	...
	if (argc > 2) {
		if (strcmp (argv[1], "-gs")==0) guessmode = GUESS_SIMPLE;
			...
			if (strcmp (argv[i], "-labeltab")==0) {
				for (i+=1; i<argc-1; i++) {
					if (sscanf (argv[i], "%x,%x", &from, &to) != 2) {
						fprintf (stderr, "Malformed -labeltab address.\n");
						break;
					} else {
						fprintf (stderr, "Labeltable 0x%x to 0x%x\n", from, to);
						for (; from <= to; from++) {
							labels[from] |= F_LABEL_TABLE;
						}
					}
				}
			}
